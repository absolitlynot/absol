...
const room = {
  ...
  maxFood: ((c.WIDTH * c.HEIGHT) / 5000000) * c.FOOD_AMOUNT,
  isInRoom: location => {
    return (
      location.x >= 0 &&
      location.x <= c.WIDTH &&
...
class io_fleeAtLowHealth extends IO {
  constructor(b) {
    super(b);
    this.fear = util.clamp(ran.gauss(0.5, 0.15), 0.1, 0.7);
  }
    ...
    connect: (() => {
      ...
      const spawn = (() => {
        ...
        return (socket, name) => {
          ...
          body.sendMessage("You are inulnerable until you move or shoot.");
  ...
  function poison(my) {
    entities.forEach(function(element) {
      ...
      if (element.poisoned && element.type == "mothership") {
        ...
        if (!element.invuln && !element.invinc) {
          // do u know what local variables are
...
var maintainloop = (() => {
  ...
  let makenpcs = (() => {
    ...
    return () => {
      ...
      if (bots.length < c.BOTS) {
        let tank = [];
        let tier1 = [];
        Class.mortal.UPGRADES_TIER_1.forEach(e => {
          tier1.push(e);
          tank.push(e);
        });
        let tier2 = [];
  ...
  let makefood = (() => {
    ...
    let makeNestFood = () => {
      ...
      placeNewFood(spot, 0.003 * room.width, 3, true);
    };
...
let server = http.createServer((req, res) => {
  ...
  switch (pathname) {
    case "/":
      res.writeHead(200);
      res.end(
        "<!DOCTYPE html><h3>Arras</h3><button onclick=\"window.location.href = 'https://arras.io/#host=arrastic.glitch.me';\">Open (test server)</button><button onclick=\"window.location.href = 'https://arras.io/#host=arrastic.herokuapp.com';\">Open (main server)</button>"
      );
      break;
    case "/mockups.json":
